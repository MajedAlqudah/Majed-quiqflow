...
//usage example for Problem 1
var transform = function(element, index, array) {
    return array[index] + index + element;
};
console.log(myMap(["a", "b", "c"], transform)); //should return ['a0a','b1b','c2c'];
// OR we can use this 
...
//Problem 3
function TrickOrTreat(callback) {
    setTimeout(function() {
        const answer = "callback kind is better than syncro kind";
        callback(answer);
    }, 1000);
}
//usage example for Problem 3
TrickOrTreat(function(answer) {
    console.log(answer);
});
